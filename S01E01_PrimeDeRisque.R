...
# Create charts
#-------------------------------------------------------------------------------------
# Important dates; Bankruptcy of Lehman Brothers, removal of CHF-EUR peg
...
# First graph: government bond yields, investment-grade enterprise bond yields
p <- ts_df(
  ts_span(
    ts_c(
      `Confédération` = Gov1,
...
# Third graph: Openings of bankruptcy procedures
head(RPBBB1) # Daily
head(Faillite) # Monthly
head(ts_regular(Faillite))

ts_regular(Faillite, fill = NA)
